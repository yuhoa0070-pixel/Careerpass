...
## URLs: always use absolute paths for assets

The site uses real clean URLs (`/schools`, `/school/rupp`, `/career/<id>`,
etc.) via the History API + a Vercel rewrite (`vercel.json`) that serves
`index.html` for any unmatched path — there's no per-page HTML, it's one
document whose JS (`app.js`) swaps which `.view` section is visible and
updates the URL to match.

This means **any relative `src`/`href` breaks on a nested route**: a plain
`src="public/logos/foo.png"` resolves against the *current URL*, so it
works when you're at `/` but resolves to the wrong place (e.g.
`/school/public/logos/foo.png`) when you're at `/school/foo` — and
silently 404s into the SPA fallback, which serves back `index.html`,
which the browser then chokes on trying to parse as an image/script/etc.
**Always write asset paths with a leading `/`** (`/public/logos/foo.png`,
`/app.js`, `/styles.css`), whether in `index.html` or generated by
`app.js` template strings (see `schoolLogo()`, `initHomeLogos()`, the
mascot's `POSES` map).

If you add a new top-level view or a new kind of detail-page route,
update `parseRouteFromUrl()` in `app.js` (reads the path/query into a
route) and the URL-writing calls in `showView()`/`openSchool()`/
`openCareer()` (write the path back) — they're two ends of the same
contract and need to stay in sync.
